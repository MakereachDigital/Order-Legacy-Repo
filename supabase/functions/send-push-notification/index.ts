...
async function generateVapidJWT(endpoint: string, vapidPrivateKey: string, vapidPublicKey: string): Promise<string> {
  ...
  const subject = 'mailto:push@delivery-picker.app';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const subscription of subscriptions) {
      try {
        console.log(`[Push] Processing subscription: ${subscription.endpoint.substring(0, 50)}...`);
        ...
        console.log(`[Push] Push service response: ${response.status}`);
        ...
        if (response.ok || response.status === 201) {
          ...
          console.error(`[Push] Error response: ${response.status} - ${errorText}`);
        }
      } catch (error) {
